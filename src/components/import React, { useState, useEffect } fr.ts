...
export default function EditProductScreen({ route, navigation }) {
  ...
  const initializeFields = (data) => {
    ...
    return {
      name: { value: data.title || data.product_title || '', editing: false },
      brand: { value: data.brand || '', editing: false },
      model: { value: data.model || '', editing: false },
      year: { value: data.manufacturing_year || data.year || '', editing: false },
      equipment_description: {
        value: data.description || data.equipment_description || '',
        editing: false,
      },
      parent_category: {
        value: data.category || data.parent_category || '',
        editing: false,
      },
      condition: { value: data.item_condition || data.condition || 'New/Unused', editing: false },
      operation_status: {
      ...
      original_price: {
        value: data.price || data.original_price || '0',
        editing: false,
      },
      product_type: { value: data.product_type || 'Marketplace', editing: false },
      auction_start_date: { value: data._yith_auction_for || data.auction_start_date || '', editing: false },
      auction_end_date: { value: data._yith_auction_to || data.auction_end_date || '', editing: false },
      auction_start_price: { value: data._yith_auction_start_price || data.auction_start_price || '0', editing: false },
      auction_group: { value: data.auction_group || '', editing: false },
      auction_currency: { value: data.auction_currency || data.currency || 'USD ($)', editing: false },
      reserve_price: { value: data._yith_auction_reserve_price || data.reserve_price || '', editing: false },
      item_location: { value: data.item_location || '', editing: false },
      ...
      product_cat: { value: data.category || '', editing: false },
      subcategory: { value: data.subcategory || '', editing: false },
    };
  };
  ...
  useEffect(() => {
    if (listing) {
      // Use existing listing data
      const initialFields = initializeFields(listing);
  ...
  const fetchLocations = async () => {
    try {
      ...
      setLocations(['New York, NY', 'Los Angeles, CA', 'Chicago, IL', 'Other']);
    } finally {
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      // Add form fields
      updateData.append('product_title', fields.name?.value || '');
      ...
      updateData.append('category', fields.parent_category?.value || fields.product_cat?.value || '');
      updateData.append('item_location', fields.item_location?.value || '');
      updateData.append('product_type', (fields.product_type?.value || 'marketplace').toLowerCase());
      updateData.append('currency', fields.currency?.value || 'USD ($)');
      updateData.append('price', fields.original_price?.value || '0');

      // Add auction-specific fields if product type is auction
      if (fields.product_type?.value === 'Auction') {
        updateData.append('_yith_auction_for', fields.auction_start_date?.value || '');
        updateData.append('_yith_auction_to', fields.auction_end_date?.value || '');
        updateData.append('_yith_auction_start_price', fields.auction_start_price?.value || '0');
        updateData.append('_yith_auction_reserve_price', fields.reserve_price?.value || '');
      }
          ...
          <View style={styles.card}>
            <View style={styles.fieldContainer}>
              ...
              <CustomDropdown
                ...
                selectedValue={fields.parent_category?.value || fields.product_cat?.value || ''}
                onSelect={(value) => {
                  onChangeText('parent_category', value);
                  onChangeText('product_cat', value);
                }}
